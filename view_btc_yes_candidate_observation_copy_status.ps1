...
Write-Host "=== BTC Yes candidate observation copy process ==="
Get-CimInstance Win32_Process |
  Where-Object { $_.CommandLine -like "*btc_yes_candidate_observation_copy_sim.py*" } |
  Select-Object ProcessId,CommandLine |
  Format-Table -AutoSize

Write-Host "`n=== Recent console ==="
$console = Join-Path $workdir "btc_yes_candidate_observation_copy_console.log"
...
$errors = Join-Path $workdir "btc_yes_candidate_observation_copy_error.log"
...
$trades = Join-Path $workdir "btc_yes_candidate_observation_copy_sim.csv"
