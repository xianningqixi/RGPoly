#!/usr/bin/env python3
"""Mirror BTC directional copy entries as a pre-live shadow validator.

This is simulation only. It does not scan wallets, read private keys, or place
orders. It watches the main BTC directional audit log and mirrors each new
main-strategy entry with a larger live-like stake to test depth, slippage, and
final-only PnL.
"""
...
import btc_directional_wallet_copy_sim as base
...
def mirror_id(source_id: str) -> str:
    return f"btc_live_shadow_mirror:{source_id}"
...
def append_reject(args: argparse.Namespace, source: dict[str, str], reason: str, extra: dict[str, Any] | None = None) -> None:
    ...
    extra = extra or {}
    detected = base.parse_ts(source.get("detected_time"))
    age = ""
    if detected:
...
def eligible_source_rows(args: argparse.Namespace) -> list[dict[str, str]]:
    rows = base.read_latest_rows(args.source_audit_log)
    ...
    for row in rows.values():
        ...
        if cutoff and ts and ts < cutoff:
            continue
        eligible.append(row)
    eligible.sort(key=lambda row: base.parse_ts(row.get("detected_time")) or base.parse_ts(row.get("audit_ts")) or datetime.min.replace(tzinfo=timezone.utc))
    return eligible


def open_exposure(args: argparse.Namespace) -> tuple[int, float]:
    return base.open_exposure(args.log, base.latest_strategy_cutoff(args.strategy_key))


def market_exposure(args: argparse.Namespace, slug: str) -> float:
    rows = base.read_latest_rows(args.log)
    cutoff = base.latest_strategy_cutoff(args.strategy_key)
    total = 0.0
    for row in rows.values():
        ...
        if row.get("status") not in {"OPEN", "MARK"}:
            continue
        ts = base.parse_ts(row.get("ts"))
        if cutoff and ts and ts < cutoff:
            continue
        total += base.fnum(row.get("stake"))
    return total
...
def mirror_source(source: dict[str, str], args: argparse.Namespace) -> bool:
    ...
    base.append_row(
        args.log,
        {
            ...
            "id": trade_id,
            "wallet_name": source.get("wallet_name", ""),
            "source_ts": source.get("signal_time", ""),
            "side": "BUY",
            "outcome": outcome,
    ...
    args.alert_file.write_text(
        (
            "BTC DIRECTIONAL LIVE SHADOW MIRROR\n"
            f"time: {base.now_utc()}\n"
    ...
    print(
        f"BTC live shadow mirror: {source.get('wallet_name', '')} {outcome} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--max-source-to-ask-gap", type=float, default=0.035)
    ...
    parser.add_argument("--source-audit-log", type=Path, default=Path("btc_directional_trade_audit.csv"))
    parser.add_argument("--source-strategy", action="append", default=["btc_directional_copy", "btc_directional_wallet_copy"])
    parser.add_argument("--log", type=Path, default=Path("btc_directional_live_shadow_sim.csv"))
    parser.add_argument("--audit-log", type=Path, default=Path("btc_directional_live_shadow_trade_audit.csv"))
    parser.add_argument("--execution-audit-log", type=Path, default=Path("btc_directional_live_shadow_execution_audit.csv"))
    parser.add_argument("--reject-log", type=Path, default=Path("btc_directional_live_shadow_rejects.csv"))
    parser.add_argument("--seen", type=Path, default=Path("btc_directional_live_shadow_mirror_seen.json"))
    parser.add_argument("--alert-file", type=Path, default=Path("latest_btc_directional_live_shadow_alert.txt"))
    parser.add_argument("--strategy-key", default="btc_directional_live_shadow")
    ...
    # Accepted for compatibility with the old launcher. They are intentionally unused.
    parser.add_argument("--min-price", type=float, default=0.0)
    parser.add_argument("--max-price", type=float, default=1.0)
    parser.add_argument("--yes-max-price", type=float, default=1.0)
    parser.add_argument("--min-source-usdc", type=float, default=0.0)
    parser.add_argument("--max-wallet-market-entries", type=int, default=1)
    parser.add_argument("--max-hours-to-end", type=float, default=0.0)
    return parser
...
def main() -> int:
    ...
    while True:
        try:
            ...
            if args.once:
                return 0
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] BTC live shadow mirror scan | opened {opened}", flush=True)
        except KeyboardInterrupt:
            return 130
        except Exception as exc:
            print(f"BTC live shadow mirror error: {exc}", flush=True)
