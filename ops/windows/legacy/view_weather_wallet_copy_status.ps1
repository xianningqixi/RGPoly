...
Write-Host "=== Weather wallet copy process ==="
Get-CimInstance Win32_Process |
  Where-Object { $_.CommandLine -like "*weather_wallet_copy_sim.py*" } |
  Select-Object ProcessId,CommandLine |
  Format-Table -AutoSize

Write-Host "`n=== Recent console ==="
$console = Join-Path $workdir "weather_wallet_copy_console.log"
...
$errors = Join-Path $workdir "weather_wallet_copy_error.log"
...
$trades = Join-Path $workdir "weather_wallet_copy_sim.csv"
if (Test-Path $trades) {
